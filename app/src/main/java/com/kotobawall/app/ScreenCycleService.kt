...
// enabled = remembered user choice; running = this process currently hosts the service.
...
object ScreenCycleController {
 ...
 @Synchronized fun initialize(context: Context) {
  if (!initialized) {
   mutable.value = CycleStatus(enabled = preferences(context).getBoolean("enabled", false))
   initialized = true
  }
 }
 ...
 fun start(context: Context) {
  ...
  check(NotificationManagerCompat.from(context).areNotificationsEnabled()) {
   "Allow Kotoba Wall notifications in Android settings before starting screen-off updates."
  }
  if (Build.VERSION.SDK_INT >= 26) {
   ...
   check(channel == null || channel.importance != NotificationManager.IMPORTANCE_NONE) {
    "Enable the Screen-off updates notification channel in Android settings."
   }
  }
  preferences(context).edit().putBoolean("enabled", true).apply()
  mutable.value = CycleStatus(enabled = true)
 ...
 fun stop(context: Context) {
  ...
  mutable.value = CycleStatus()
  context.stopService(Intent(context, ScreenCycleService::class.java))
 }
...
class ScreenCycleService : Service() {
 ...
 private val receiver = object : BroadcastReceiver() {
  override fun onReceive(context: Context, intent: Intent) {
   ...
   // Coalesce rapid toggles while one bitmap is being written; never queue a burst.
   ...
   updateJob = job
   // This also executes when cancellation happens before the coroutine starts.
 ...
 override fun onCreate() {
  super.onCreate()
  ScreenCycleController.initialize(this)
  try {
   if(Build.VERSION.SDK_INT >= 26) {
    getSystemService(NotificationManager::class.java).createNotificationChannel(
     NotificationChannel(CHANNEL,"Screen-off updates",NotificationManager.IMPORTANCE_LOW).apply {
      description = "Visible while vocabulary wallpaper updates are active."
     }
    )
   }
   ...
   val notification = NotificationCompat.Builder(this,CHANNEL)
    ...
    .setContentTitle("Kotoba Wall is active")
    .setContentText("Preparing a new word when your screen turns off")
   ...
   ScreenCycleController.failure(e.message ?: "Screen-off updates could not start.")
   stopSelf()
  }
 }
 ...
 override fun onDestroy() {
  ...
  ScreenCycleController.stopped(this)
  super.onDestroy()
 }
}
