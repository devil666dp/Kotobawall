...
data class WallSettings(
 ...
 val favorites: Set<String> = emptySet(),val favoritesOnly: Boolean=false,val shuffle: Boolean=false,val rotateWallpaper: Boolean=false
)
object WallMath {
 fun nextIndex(current: Int,count: Int): Int {require(count>0);return (current.coerceIn(0,count-1)+1)%count}
